...
A payment that works is the easy case. This documents what happens when the
agent is confused, manipulated, or buggy. Below is a set of attacks against the
server and the result of each.
...
| A3 | Charge an MSISDN not on the allowlist (hallucinated number) | Rejected before any API call | `reason_code=msisdn_not_allowlisted` | yes |
...
- The suite caught a real bug. An early version excluded `DRY_RUN` transactions
  from the daily-limit counters, so the A2 splitting attack got through in
  dry-run mode. The test caught it; the fix makes the limits apply in dry-run
  too, so the scorecard covers the default operating mode. See the
